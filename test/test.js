...
Client.prototype.query = function (sql, values, callback) {
  ...
  var out = { sql, values, callback };
    // prepared statement call with 2 arguments
  if (isUndefined(callback) && isFunction(values) && isPlainObject(sql)) {
...
describe('node-postgres-named', () => {
  ...
  describe('Parameter translation', () => {
    ...
    it('Lexicographic order of parameter keys differs from order of appearance in SQL string', () => {
  ...
  describe('Monkeypatched Dispatch', () => {

    it('Call with original signature results in unchanged call to original function', () => {

      var sql = 'SELECT name FORM person WHERE name = $1 AND tenure <= $2 AND age <= $3';
      var values = ['Ursus Oestergardii', 3, 24];
      var results = client.query(sql, values);

      assert.equal(results.sql, sql);
      assert.deepEqual(results.values, values);
      assert.equal(results.callback, undefined);

    });

    it('Call with no values results in unchanged call to original function', () => {
    ...
    it('Named parameter call dispatched correctly', () => {
      ...
      var results = client.query(sql, values, emptycb);
      ...
      assert.equal(results.callback, emptycb);

    });

    it('Prepared statement call', () => {
      ...
      assert.equal(results.sql, 'SELECT name FORM person WHERE name = $2 AND tenure <= $3 AND age <= $1');
      assert.deepEqual(results.values, [24, 'Ursus Oestergardii', 3]);
      assert.equal(results.callback, callback);
  ...
  describe('Arrays', () => {
    ...
    it('should accept arrays as values in k/v object', () => {

      var sql  = 'SELECT * FROM table WHERE name LIKE $name AND id = ANY($ids::int[])';
      var vals = {
        ids: [13, 37],
        name: 'jarse'
      };

      var results = client.query(sql, vals, emptycb);
      assert.equal(results.sql, 'SELECT * FROM table WHERE name LIKE $2 AND id = ANY($1::int[])');
      assert.deepEqual(results.values, [[13, 37], 'jarse']);
